...
const defaultReducer = (state, action) => {
	...
	switch (action.type) {
		case 'increment': return { ...state, count: state.count + change };
		case 'decrement': return { ...state, count: state.count - change };
		case 'reset': return { ...state, count: 0 };
		default: throw new Error(`Type d'action non supporté: ${action.type}`);
	};
};
