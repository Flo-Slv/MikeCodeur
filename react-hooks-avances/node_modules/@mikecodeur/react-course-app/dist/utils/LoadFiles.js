...
function loadFiles({
  ...
  const fileInfo = _glob.default.sync('src/{exercise,final,demo,examples}/**/*.+(js|html|jsx|ts|tsx|md|mdx|json)', {
    cwd,
    ...rest
  ...
  .map(filePath => {
    ...
    const [firstLine, secondLine = '', thirdLine = ''] = contents.split(/\r?\n/);
    let title = 'Unknown';
    let extraCreditTitle = 'Unknown';
    const isExtraCredit = name.includes('.bonus-');
    const fallbackMatch = {
      groups: {
        title: ''
      }
    ...
    if (parentDir === 'final' || parentDir === 'exercise') {
      if (ext === '.js' || ext === '.tsx' || ext === '.ts') {
        ...
        var _firstLine$match3, _titleMatch$groups$ti3, _titleMatch$groups3, _thirdLine$match, _descriptionMatch$gro, _descriptionMatch$gro2;
        ...
        const descriptionMatch = (_thirdLine$match = thirdLine.match(/### 💡 (?<title>.*)$/)) != null ? _thirdLine$match : fallbackMatch;
        description = (_descriptionMatch$gro = (_descriptionMatch$gro2 = descriptionMatch.groups) == null ? void 0 : _descriptionMatch$gro2.title.trim()) != null ? _descriptionMatch$gro : description;
      }
    }
    ...
    return {
      ...
      fullFilePath,
      filePath,
      isolatedPath: filePath.replace('src', '/alone'),
      ext,
      filename: name,
      type,
  ...
  fileInfo.sort((a, b) => {
    // change order so that shorter file names (01) are before longer (01.bonus-01)
